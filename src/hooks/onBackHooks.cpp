...
class $modify(SecretRewardsLayer) {
	...
	#ifndef GEODE_IS_MOBILE
	void keyBackClicked() {
		if (!Variables::isFromRedash || m_secondaryLayer) return SecretRewardsLayer::keyBackClicked();
		CCScene* sc = MenuLayer::scene(false);
		CCDirector::sharedDirector()->replaceScene(CCTransitionFade::create(0.5f, sc));
        Variables::isFromRedash = false;
	}
	...
	void onBack(cocos2d::CCObject* sender) {
        if (!Variables::isFromRedash || m_secondaryLayer) return SecretRewardsLayer::onBack(sender);
		CCScene* sc = MenuLayer::scene(false);
		CCDirector::sharedDirector()->replaceScene(CCTransitionFade::create(0.5f, sc));
        Variables::isFromRedash = false;
	}
